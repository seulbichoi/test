##############################
lag_dt1 <- function (y, maxlag, keeporig = TRUE) {
...
lag_dt2 <- function (x, maxlag) {
    ...
    for (i in 1:ncol(x)) {
        m[, 1:(maxlag + 1) + (i - 1) * (maxlag + 1)] <- lag_dt1(x[, i], maxlag = maxlag)
    }
    thenames <- character()
    for (i in 1:ncol(x)) {
...
forward <- function(x, y, model, xregpred, object= object, i, f = 7) {
        ...
        if (f > 1 & object$seas_method == "dummies") {
...
forecast_ap <- function (object, h = 42, xreg = NULL, lambda = object$lambda) {
    if (!is.null(xreg)) {
        ...
        h <- nrow(xreg)
        xreg2 <- lag_dt2(rbind( object$origxreg,xreg), maxlag = object$maxlag)
        # xreg2 <- lag_dt2(rbind(xreg, object$origxreg), maxlag = object$maxlag)
        nn <- nrow(xreg2)
...
xgboost.forecast <- function (y, h = 42,xreg = NULL, pred_xreg = NULL,
    ...
    ts_clean = FALSE, seas_method = c("decompose", 'dummies', "none"),
...
###### Check Data format
    ...
    x[, 1:maxlag] <- lag_dt1(target_y, maxlag, keeporig = FALSE)
    ...
    if (!is.null(xreg)) {
        xreg <- lag_dt2(xreg, maxlag = maxlag)
